...
namespace ContentRatingAPI.Infrastructure.ContentFileManagers.FileSavers
{
    public partial class HLSVideoSaver : FileSaverBase
    {
        ...
        public HLSVideoSaver(IOptions<ContentFileOptions> options, IOptions<FFMPEGOptions> FFMPEGOptions) : base(options)
        {
        ...
        public override async Task<SavedContentFileInfo> SaveFile(Guid fileId, string fileExtension, byte[] data, CancellationToken cancellationToken = default)
        {
            ...
            return new SavedContentFileInfo(fileId, DateTime.UtcNow, Path.Combine(filePath, fileName + ".m3u8"), ContentRating.Domain.Shared.Content.ContentType.Video);
        }
        ...
        private async Task<VideoInfo> GetVideoInfo(byte[] videoData, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                Regex regexVideoBitrate = VideoRegex();
                Regex regexFps = FPSRegex();
                Regex regexAudioBitrate = AudioRegex();
                ...
                {
                    string bitrateUnitVideo = matchVideoBitrate.Groups[2].Value;
                ...
                {
                    string bitrateUnitAudio = matchAudioBitrate.Groups[2].Value;
