...
namespace ContentRating.IntegrationTests.DataHelpers
{
    internal static class ContentEstimationListEditorGenerator
    {
        ...
        public static List<ContentEstimationListEditor> ContentEstimationListEditors(int count, Guid? editorId = null)
        {
            var editors = new List<ContentEstimationListEditor>();

            for (int i = 0; i < count; i++)
            {
                ContentEstimationListEditor editor = ContentEstimationListEditor(editorId);
        ...
        public static ContentEstimationListEditor ContentEstimationListEditor(Guid? editorId = null)
        {
            var id = Guid.NewGuid();
            var creator = GenerateEditor(editorId);
            var name = _faker.Lorem.Sentence(3, 3);
            var roomEditor = Domain.AggregatesModel.ContentEstimationListEditorAggregate.ContentEstimationListEditor.Create(id, creator, name);
        ...
        private  static List<ContentData> GenerateContentList(ContentEditor editor, int count = 5)
        {
