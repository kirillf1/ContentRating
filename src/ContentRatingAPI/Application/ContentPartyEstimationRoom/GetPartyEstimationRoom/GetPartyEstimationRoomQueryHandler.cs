...
namespace ContentRatingAPI.Application.ContentPartyEstimationRoom.GetPartyEstimationRoom
{
    public class GetPartyEstimationRoomQueryHandler : IRequestHandler<GetPartyEstimationRoomQuery, Result<PartyEstimationRoomResponse>>
    {
        ...
        {
            partyRatingRoomCollection = mongoContext.GetCollection<ContentPartyEstimationRoomAggregate>(options.Value.ContentPartyEstimationRoomCollectionName);
        ...
        {
            var query = from estimationRoom in partyRatingRoomCollection.AsQueryable().Where(c => c.Id == request.RoomId)
