...
{
    public class ContentPartyEstimationHubService
            ...
            IAsyncDisposable
    {
        ...
        public async Task ConnectAsync(Guid roomId)
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Connected to SignalR hub for estimation room {RoomId}",
                    roomId
                );
            }
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to connect to SignalR hub for estimation room {RoomId}",
                    roomId
                );
                throw;
            }
        ...
        public async Task DisconnectAsync()
        {
            if (_hubConnection != null)
            {
                try
                {
                    ...
                    _logger.LogInformation(
                        "Disconnected from SignalR hub for estimation room {RoomId}",
                        _currentRoomId
                    );
                }
                catch (Exception ex)
        ...
        public async Task EstimateContentAsync(Guid contentRatingId, double newRating)
        {
            ...
            try
            {
                ...
                _logger.LogDebug(
                    "Sent rating {Rating} for content {ContentRatingId}",
                    newRating,
                    contentRatingId
                );
            }
        ...
        private void SetupEventHandlers()
        {
            ...
            _hubConnection.On<Guid, Guid, double>(
                ...
                {
                    _logger.LogDebug(
                        "Received RatingChanged event: Rater {RaterId}, Rating {RatingId}, Score {Score}",
                        raterId,
                        ratingId,
                        score
                    );
            ...
            _hubConnection.On<Guid, string, double>(
                ...
                {
                    _logger.LogDebug(
                        "Received RaterInvited event: {RaterId} - {RaterName}",
                        newRaterId,
                        raterName
                    );
            ...
            _hubConnection.On<double, double>(
                ...
                {
                    _logger.LogDebug(
                        "Received RatingRangeChanged event: {MinRating} - {MaxRating}",
                        minRating,
                        maxRating
                    );
            ...
            _hubConnection.Reconnected += async (connectionId) =>
            {
                ...
                if (_currentRoomId.HasValue)
                {
                    try
                    {
                        ...
                        _logger.LogInformation(
                            "Successfully rejoined estimation room {RoomId} after reconnection",
                            _currentRoomId.Value
                        );
                    }
                    catch (Exception ex)
