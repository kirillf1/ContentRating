...
namespace ContentRatingAPI.Hubs.Filters
{
    public class LoggingHubFilter : IHubFilter
    {
        ...
        {
            logger.LogInformation("Calling hub method: {method}, connectionId: {connectionId}",
                invocationContext.HubMethodName, invocationContext.Context.ConnectionId);
            try
            {
                var result = await next(invocationContext);

                logger.LogInformation("Calling hub method: {method} completed, connectionId: {connectionId}",
                    invocationContext.HubMethodName, invocationContext.Context.ConnectionId);
            ...
            {
                logger.LogError("Exception calling {method}, connectionId: {connectionId}, error: {ex}",
                    invocationContext.HubMethodName, invocationContext.Context.ConnectionId, ex);
                throw;
            }
        }
        ...
        {
            logger.LogInformation("Connection created, connectionId: {connectionId}",
