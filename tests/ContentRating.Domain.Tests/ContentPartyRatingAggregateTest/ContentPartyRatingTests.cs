...
namespace ContentRating.Domain.Tests.ContentPartyRatingAggregateTest
{
    public class ContentPartyRatingTests
    {
        ...
        public void InviteRater_NewRater_RaterAdded()
        {
            ...
            Assert.Equal(
                contentRating.AverageContentScore,
                specification.MinScore
            );
        ...
        public void RemoveRater_ExistingRater_RaterRemoved()
        {
            ...
            Assert.DoesNotContain(
                rater.RaterId,
                contentRating.RaterScores.Keys
            );
        }
        ...
        public void AddNewRater_ContentEstimated_ThrowForbiddenRatingOperationException()
        {
            ...
            Assert.Throws<ForbiddenRatingOperationException>(
        ...
        public void EstimateContent_ValidScore_ScoreChanged(double value)
        {
            ...
            var contentRatingChanged = contentRating
                .DomainEvents.OfType<ContentRatingChangedDomainEvent>()
                .Single();
            Assert.Equal(
                contentRating.AverageContentScore,
                contentRatingChanged.AverageContentScore
            );
            Assert.Equal(contentRating.RoomId, contentRatingChanged.RoomId);
            Assert.Equal(
                expectedScore,
                contentRating.RaterScores[newRater.RaterId]
            );
        }
        ...
        [InlineData(-2)]
        [InlineData(10.1)]
        public void EstimateContent_OutOfRangeScore_ThrowForbiddenRatingOperationException(
            double value
        )
        {
            var specification = CreateContentRatingSpecification(0, 10);
        ...
        public void EstimateContent_ForeignRatingWithoutPermission_ThrowForbiddenRatingOperationException()
        {
            ...
            var defaultRater = new ContentRater(
                Guid.NewGuid(),
                RaterType.Default
            );
        ...
        private static ContentRatingSpecification CreateContentRatingSpecification(
            double minScore = 0,
            double maxScore = 10
        )
        {
            return new ContentRatingSpecification(
                new Score(minScore),
                new Score(maxScore)
            );
        }
