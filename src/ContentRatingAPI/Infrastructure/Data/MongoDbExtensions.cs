    ...
    public static class MongoDbExtensions
    {
        public static IServiceCollection AddMongoDbStorage(this IHostApplicationBuilder builder)
        {
            ...
            builder.Services.AddTransient<
                IMongoCollectionIndexFactory,
                ContentPartyRatingIndexFactory
            >();
            builder.Services.AddTransient<
                IMongoCollectionIndexFactory,
                ContentPartyEstimationIndexFactory
            >();
            builder.Services.AddTransient<
                IMongoCollectionIndexFactory,
                ContentEstimationListIndexFactory
            >();

            builder.Services.AddMemoryCache();
            builder.Services.AddTransient<
                GenericCacheBase<ContentPartyEstimationRoom>,
                GenericInMemoryCache<ContentPartyEstimationRoom>
            >();
            builder.Services.AddTransient<
                GenericCacheBase<ContentEstimationListEditor>,
                GenericInMemoryCache<ContentEstimationListEditor>
            >();
            builder.Services.AddTransient<
                GenericCacheBase<ContentPartyRating>,
                GenericInMemoryCache<ContentPartyRating>
            >();

            builder.Services.AddTransient<
                IContentEstimationListEditorRepository,
                ContentEstimationListEditorRepository
            >();
            builder.Services.Decorate<
                IContentEstimationListEditorRepository,
                CachingContentEstimationListEditorRepository
            >();
            builder.Services.AddTransient<
                IContentPartyEstimationRoomRepository,
                ContentPartyEstimationRoomRepository
            >();
            builder.Services.Decorate<
                IContentPartyEstimationRoomRepository,
                CachingPartyEstimationRoomRepository
            >();
            builder.Services.AddTransient<
                IContentPartyRatingRepository,
                ContentPartyRatingRepository
            >();
        ...
        private static void RegisterEntityClassMap()
        {
            ...
            BsonClassMap.RegisterClassMap<RatingRange>(classMap =>
            {
                classMap.AutoMap();
                classMap.MapCreator(c => new RatingRange(c.MaxRating, c.MinRating));
                classMap.MapProperty(c => c.MinRating).SetElementName("MinRating");
                classMap.MapProperty(c => c.MaxRating).SetElementName("MaxRating");
            });

            BsonClassMap.RegisterClassMap<Rating>(classMap =>
            {
                classMap.AutoMap();
                classMap.MapProperty(c => c.Value).SetElementName("Value");
            });

            BsonClassMap.RegisterClassMap<ContentPartyRating>(classMap =>
            {
                classMap.AutoMap();
                classMap.SetDictionaryRepresentation(
                    c => c.RaterScores,
                    DictionaryRepresentation.ArrayOfDocuments
                );
                classMap
                    .MapProperty(c => c.ContentId)
                    .SetSerializer(new GuidSerializer(GuidRepresentation.Standard));
                classMap
                    .MapProperty(c => c.RoomId)
                    .SetSerializer(new GuidSerializer(GuidRepresentation.Standard));
            });
