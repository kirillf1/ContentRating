...
using ContentRating.Web.UI.ViewModels.Shared;
    ...
    public class ContentPartyEstimationRoomViewModel : IAsyncDisposable
    {
        ...
        public async Task<bool> EstimateContentAsync(Guid contentRatingId, double newRating)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Ошибка при оценке контента {ContentRatingId} пользователем {UserId}",
                    contentRatingId,
                    CurrentUserId
                );
        ...
        public async Task<bool> DeleteContentAsync(Guid contentId)
        {
            try
            {
                ...
                if (success)
                {
                    ...
                    var contentToRemove = ContentRatings.FirstOrDefault(c =>
        ...
        )
        {
            ...
            try
            {
                ...
                if (success)
                {
                    ...
                    _snackbar.Add(
                        $"Контент оценен от имени {mockUser.DisplayName}: {newRating:F2}",
                        Severity.Success
                    );
                    _isFilterDirty = true;
                    RecalculateFilter();
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Ошибка при оценке контента {ContentRatingId} от имени mock пользователя {MockUserId}",
                    contentRatingId,
                    mockUserId
                );
                _snackbar.Add("Произошла ошибка при оценке контента", Severity.Error);
