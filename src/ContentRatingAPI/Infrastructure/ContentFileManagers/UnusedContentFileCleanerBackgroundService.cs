...
namespace ContentRatingAPI.Infrastructure.ContentFileManagers
{
    public class UnusedContentFileCleanerBackgroundService : BackgroundService
    {
        ...
        public UnusedContentFileCleanerBackgroundService(IServiceScopeFactory serviceScopeFactory, 
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                using (IServiceScope scope = serviceScopeFactory.CreateScope())
                {
                    ...
                    var removeResult = await contentFileManager.RemoveUnusedSavedContentFiles(TimeSpan.FromHours(fileOptions.Value.OldCheckedFileTakeIntervalInHours), stoppingToken);
